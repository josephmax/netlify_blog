...
gulp.task("image", () => {
  ...
  return gulp
    ...
    .pipe(gulp.dest(destDir))
    .pipe(bs.stream({ match: "*.{png,jpeg,jpg}" }));
});
...
//devDir => prodDir
gulp.task("rev", () => {
  const revExts = "png,svg,jpg,jpeg,css,js";
  return gulp
    .src(`${devDir}/**/*.{${revExts}}`)
    .pipe(rev())
...
gulp.task('generate-service-worker', function(callback) {
  ...
  swPrecache.write(`${rootDir}/service-worker.js`, {
    staticFileGlobs: [rootDir + '/**/*.{js,html,css,png,jpg,jpeg,gif,svg,eot,ttf,woff,json,woff2}'],
    stripPrefix: rootDir,
    runtimeCaching: [{
      urlPattern: "/*",
      handler: 'cacheFirst',
